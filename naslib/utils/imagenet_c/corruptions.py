...
def shot_noise(x, severity=1):
    ...
    return x.T
    # return transforms.ToTensor()(x.T)
...
def frost(x, severity=1):
    ...
    x = np.clip(c[0] * np.array(x) + c[1] * frost_rescaled, 0, 255)
    return x.T
...
def brightness(x, severity):
    ...
    severity = float_parameter(sample_level(severity), 1.8) + 0.1
    x = ImageEnhance.Brightness(x).enhance(severity)
...
def pixelate(x, severity=1):
    ...
    x = x.resize((int(x_shape[1] * c), int(x_shape[2] * c)), Image.BOX)

    x = x.resize((x_shape[1], x_shape[0]), Image.BOX)
    x = transforms.ToTensor()(x)
    x = x.numpy()
    return x
